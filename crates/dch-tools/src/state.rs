...
///
/// Keyed by the path exactly as the model supplied it: the same spelling in
/// Read and Write matches, while a differently-spelled alias of the same
/// file simply has no entry — Write then proceeds unchecked. A value of
/// `None` means the path was touched but its `mtime` could not be measured
/// (the `stat` failed at read time); it clears any older baseline rather
/// than leaving a stale one to be trusted.
pub type FileBaselines = BTreeMap<String, Option<SystemTime>>;

/// Record `mtime` as the model's latest known state of `path`.
///
/// A `None` `mtime` (the `stat` failed) overwrites any older baseline — the
/// newest observation wins, and an unmeasurable one is no basis for a
/// staleness verdict.
pub(crate) fn record(baselines: &mut FileBaselines, path: &str, mtime: Option<SystemTime>) {
    baselines.insert(path.to_string(), mtime);
}

/// The recorded baseline for `path`, when the latest touch measured an
/// `mtime`.
pub(crate) fn baseline(baselines: &FileBaselines, path: &str) -> Option<SystemTime> {
...
mod tests {
    ...
    fn baseline_follows_the_latest_recorded_touch() {
        ...
        record(&mut baselines, "a.rs", Some(epoch));
        record(
            &mut baselines,
            "b.rs",
        ...
        assert_eq!(
            baseline(&baselines, "a.rs"),
            Some(epoch + std::time::Duration::from_secs(10)),
            "the latest touch wins"
        );
        assert_eq!(
            baseline(&baselines, "b.rs"),
            Some(epoch + std::time::Duration::from_secs(5)),
            "other paths keep their own baselines"
        );
        assert_eq!(baseline(&baselines, "missing.rs"), None);
    }
    ...
    fn an_unmeasured_touch_clears_the_baseline() {
        ...
        let epoch = SystemTime::UNIX_EPOCH;
        record(&mut baselines, "a.rs", Some(epoch));
        record(&mut baselines, "a.rs", None);
